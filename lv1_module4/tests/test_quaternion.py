...
def test_slerp_endpoints(q_pair):
    ...
    # 부호 이중덮개: |내적| == 1 또는 회전행렬 비교
    assert np.isclose(abs(float(np.dot(q_start, q0))), 1.0, atol=1e-6)
    assert np.isclose(abs(float(np.dot(q_goal, q1))), 1.0, atol=1e-6)
    assert np.allclose(quaternion_to_matrix(q_start),
                       quaternion_to_matrix(q0), atol=1e-6)
    assert np.allclose(quaternion_to_matrix(q_goal),
                       quaternion_to_matrix(q1), atol=1e-6)
