...
from .vectors import inverse_gauss_jordan, gauss_eliminate
...
def least_squares_normal_equation(A, b):
    ...
    # 역행렬을 이용하여 해를 계산 (gauss_eliminate는 (x, steps) 반환)
    try:
...
def transform_point(T, p):
    ...
    if p.ndim != 1 or p.shape[0] != 3:
        raise ValueError("p는 (3,) 형태의 점이어야 합니다.")

    # 점을 동차좌표로 변환 (w=1)
    p_h = to_homogeneous(p, 1)
    t_p_h = T @ p_h
    transformed_p = t_p_h[:3]
